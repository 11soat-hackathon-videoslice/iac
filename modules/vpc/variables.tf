...
variable "environment" {
  ...
  validation {
    condition     = contains(["prd", "stg", "qa", "dev", "labs", "devops", "payer"], var.environment)
    error_message = "The value must be either 'prd', 'stg', 'qa', 'dev', 'labs', 'devops', 'payer'."
  }
}
...
variable "enable_vpc_flow_logs" {
  ...
  validation {
    condition     = contains([true, false], var.enable_vpc_flow_logs)
    error_message = "The value must be either 'true' or 'false'."
  }
}
